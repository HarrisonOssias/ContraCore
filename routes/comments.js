...
router.post(
  '/:id',
  [auth, [check('commentText', 'No comment text was included.').exists()]],
  async (req, res) => {
    ...
    try {
      ...
      if (!project) {
        return res.status(404).json({
          msg: 'No project found with the same id as passed in through the url'
        });
      }
...
router.delete(
  '/:id',
  [auth, [check('_id', 'Please enter a project id in the body.').exists()]],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    try {
      ...
      if (!projects) {
        return res
          .status(404)
          .json({ msg: 'No project with this id was found' });
      }
      ...
      if (deleteIndex < 0 || !deleteIndex) {
        return res
          .status(404)
          .json({ msg: 'No comment was found with this id' });
      }
